...
public class VertexModel extends AbilityModel {
    ...
    public VertexModel( int x, int y, EntityModel parent) {
        super(parent.getGlobeId(), parent.getLocalPanelModel(), x, y);
        this.parentEntityModel = parent;
        if (parentEntityModel instanceof EpsilonModel)getGlobeModel().getGlobeController().createAbilityView(id, x, y);
    }
